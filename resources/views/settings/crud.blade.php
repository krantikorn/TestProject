    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            <button wire:click="create()"
                class="bg-green-700 text-white font-bold py-2 px-4 rounded my-3">Add Settings</button>
            @if($isModalOpen)
                @include('settings.update')
            ...
            <table class="table-auto w-full">
                ...
                <tbody>
                    @foreach($settings as $k => $setting)
                        <tr>
                            <td class="border px-4 py-2">{{ $settings->firstItem() + $k }}</td>
                            <td class="border px-4 py-2">{{ $setting->name }}</td>
